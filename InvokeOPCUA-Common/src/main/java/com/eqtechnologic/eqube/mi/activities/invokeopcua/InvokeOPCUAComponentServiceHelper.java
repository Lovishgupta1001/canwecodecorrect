...
public class InvokeOPCUAComponentServiceHelper {
    ...
    private TransportInfo mapTransportClientToInfo(String transportType, TransportClientBean transportClientBean) {
        ...
        if (InvokeOPCUAConstants.OPCUA_TYPE.equalsIgnoreCase(transportType)) {
    ...
    List<eQExportEntity> getConfigLinkedResources(Map<String, Object> configData) {
        ...
        if (configData != null) {
            String strTransportName = (String) configData.get(InvokeOPCUAConstants.TRANSPORT_NAME);
    ...
    void calculateConfigRemapInfo(RemapInfo completeRemapInfo, Map<String, Object> configData) {
        ...
        if (strTransportName != null && !strTransportName.trim().isEmpty()) {
            DeployRemapBean deployRemapBean = new DeployRemapBean(eQResourceType.TRANSPORT, (String) configData.get(InvokeOPCUAConstants.TRANSPORT_NAME),
                    (String) configData.get(InvokeOPCUAConstants.TRANSPORT_NAME), null);
            Map<String, Object> otherDetails = new HashMap<>();
            otherDetails.put("transportType", InvokeOPCUAConstants.OPCUA_TYPE);
    ...
    void setConfigRemapInfo(RemapInfo completeRemapInfo, Map<String, Object> configData) {
        ...
        for (DeployRemapBean remapInfo : completeRemapInfos.getDeployRemapInfo(eQResourceType.TRANSPORT.name())) {
            if (remapInfo.getOldValue().equals(strTransportName) && remapInfo.getOtherDetails() != null){
                configData.put(InvokeOPCUAConstants.TRANSPORT_NAME, remapInfo.getNewValue());
            }
        }
    }
}
