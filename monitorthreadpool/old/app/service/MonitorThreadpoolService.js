...
export async function downloadThreadDump(errorHandler,nodeId) {
    return networkInstance.get("miMonitoring/miHealth/downloadThreadDumpByNode?nodeId="+nodeId,
    {
...
export async function getThreadDetails(executionServerName,errorHandler) {
    return networkInstance.get("miMonitoring/miHealth/getThreadsAndQueuesData?executionServerName="+executionServerName,
    {
        customErrorHandler: getCustomErrorHandler(errorHandler)
    }
    );
...
export async function getAllOperations(errorHandler) {
    return networkInstance.get("cacutility/commonAdminConsole/getAllOperations",
    {
        customErrorHandler: getCustomErrorHandler(errorHandler)
    }
    );
...
export async function getExecutionServerNames(errorHandler) {
    return networkInstance.get("miMonitoring/miHealth/getExecutionServerNames",
    {
        customErrorHandler: getCustomErrorHandler(errorHandler)
    }
    );
...
export async function abortTransactions(isForceful,threadIds,errorHandler){
    return networkInstance.post("miMonitoring/transactionService/abortThread",{
        txnIdList:threadIds,
...
export async function getDSURL(errorHandler) {
    return networkInstance.get("miMonitoring/MiMonitoringService/fetchDSURL",
    {
        customErrorHandler: getCustomErrorHandler(errorHandler)
    }
    );
