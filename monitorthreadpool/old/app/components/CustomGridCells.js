...
export const customSubStepCell = (props) => {
    ...
    const condition = (poolInfo.openConnectionsList && poolInfo.openConnectionsList.length > 0) ||
    (poolInfo.pendingRequest && poolInfo.pendingRequest.trim() !== '');
    const subStepCell = (
        <div className="ul-flex-col-container overflowing-cell-content" title={props.dataItem[constants.ThreaddetailsTableColumns.Field.subStepName]}>
            <span className="overflowing-cell-content">
                {props.dataItem[constants.ThreaddetailsTableColumns.Field.subStepName]}
            </span>
            <span className='ul-pad-1x-x'>
                {condition ? <EQULTooltipOnIcon iconClass='eQ-icon eQ-fonts-goto redirect-to-Mon-Con-Pool' onClick={()=>redirectHandler(props.dataItem.poolInfo)}type="action" size="xs" tooltipProps={tooltipIconProps} title={nls('Go_to_Monitor_Connection_Pool')} />:''}
            </span>
        </div>
    );
...
export const customTxnIdCell = (props) => {
    var dsURL = useDsURL()
    var txnId = props.dataItem[constants.ThreaddetailsTableColumns.Field.txnId];
    const pathName = `${dsURL}/DESIGNER#IDE/${txnId}`;

    const txnIdCell = (
        <div className="ul-flex-col-container overflowing-cell-content cell-link">
            <a className="overflowing-cell-content" href={pathName} target="_blank" rel="noopener noreferrer"> {txnId} </a>
        </div>
    );
