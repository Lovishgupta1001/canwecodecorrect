...
* The callback function displays a UIlayer notification with the error message based on error code
...
function getExceptionHandler(exceptionNLS, defaultMessage) {
    return (error) => {
        ...
        if(errorCode) {
            errorMessage = exceptionNLS([ `exceptions.${errorCode}`, constants.ERROR_CODE_NOT_FOUND]);
            if(errorMessage === constants.ERROR_CODE_NOT_FOUND) {
                return false;
            }
        }
        else {
            errorMessage = (defaultMessage)? defaultMessage: exceptionNLS('defaultMessage');
        }
