...
function AbortTransactions({
    ...
    const [abortTransactionsEndpointResponse, setAbortTransactionsEndpointResponse] =
        React.useState(null);
    ...
    const toggleDialog = () => {
        if (
            OperationList &&
            OperationList.includes(constants.OPERATIONS.ABORT_THREAD)
        ) {
    ...
    const abortTransactionsHandler = (forceful, abortThreadIds) => {
        abortTransactions(
            ...
            setAbortTransactionsEndpointResponse(
                abortedTransactionsStatusArray
            );
            emptyThreadIdsCallback();
        ...
        <div className="ul-pad-1x-x">
            ...
            {visible && (
                <EQULWindow
                    ...
                    size={"small"}
                >
                    <div className="ul-pad-1x-x ul-pad-2x-y">
                        <EQULTypo type="body" size="large">
                            {nls("AbortTransactions.ModalBody")}
                        </EQULTypo>
                    </div>
                    <div className="ul-pad-1x-x ul-pad-2x-b ul-pad-1x-t ul-row abort-thread-modal-options-width">
                        <div className="ul-col-sm-2">
                            <EQULRadioButton
                                checked={systematic}
                                label={nls(
                                    "AbortTransactions.ModalSystematicLabel"
                                )}
                                onChange={() => {
                                    setSystematic(!systematic);
                                    setForceful(!forceful);
                                }}
                            />
                        </div>
                        <div className="ul-col-sm-2">
                            <EQULRadioButton
                                checked={forceful}
                                label={nls(
                                    "AbortTransactions.ModalForcefulLabel"
                                )}
                                onChange={() => {
                                    setSystematic(!systematic);
                                    setForceful(!forceful);
            ...
            {!abortCheckFailed && visibleConfirmation && (
                <EQULWindow
                    ...
                    title={
                        systematic
                            ? nls("SystematicAbortConfirmation")
                            : nls("ForcefulAbortConfirmation")
                    }
                    size={"xlarge"}
                >
                    <div className="ul-pad-2x-y">
                        <EQULGrid
                            columns={columns}
                            data={
                                abortTransactionsEndpointResponse
                                    ? abortTransactionsEndpointResponse
                                    : []
                            }
                        />
